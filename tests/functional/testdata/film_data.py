DETAILED_FILM_DATA = {
    'id': 'cddf9b8f-27f9-4fe9-97cb-9e27d4fe3394',
    'title': 'Star Wars: Episode VII - The Force Awakens',
    'imdb_rating': 7.9,
    'description': '30 years after the defeat of Darth Vader and the Empire, Rey, '
                   'a scavenger from the planet Jakku, finds a BB-8 droid that knows '
                   'the whereabouts of the long lost Luke Skywalker. Rey, as well as a '
                   'rogue stormtrooper and two smugglers, are thrown into the middle of a '
                   'battle between the Resistance and the daunting legions of the First Order.',
    'genre': [{
        'id': '120a21cf-9097-479e-904a-13dd7198c1dd',
        'name': 'Adventure'
    }, {
        'id': '3d8d9bf5-0d90-4353-88ba-4ccc5d2c07ff',
        'name': 'Action'
    }, {
        'id': '6c162475-c7ed-4461-9184-001ef3d9f26e',
        'name': 'Sci-Fi'
    }],
    'actors': [{
        'id': '26e83050-29ef-4163-a99d-b546cac208f8',
        'name': 'Mark Hamill'
    }, {
        'id': '2d6f6284-13ce-4d25-9453-c4335432c116',
        'name': 'Adam Driver'
    }, {
        'id': '5b4bf1bc-3397-4e83-9b17-8b10c6544ed1',
        'name': 'Harrison Ford'
    }, {
        'id': 'b5d2b63a-ed1f-4e46-8320-cf52a32be358',
        'name': 'Carrie Fisher'
    }],
    'writers': [{
        'id': '3217bc91-bcfc-44eb-a609-82d228115c50',
        'name': 'Lawrence Kasdan'
    }, {
        'id': 'a1758395-9578-41af-88b8-3f9456e6d938',
        'name': 'J.J. Abrams'
    }, {
        'id': 'a5a8f573-3cee-4ccc-8a2b-91cb9f55250a',
        'name': 'George Lucas'
    }, {
        'id': 'cec00f0e-200b-4b48-9ed1-2f8fc3c67427',
        'name': 'Michael Arndt'
    }],
    'directors': [{
        'id': 'a1758395-9578-41af-88b8-3f9456e6d938',
        'name': 'J.J. Abrams'
    }]
}

DETAILED_FILM_DATA_GOT_FROM_CACHE = {
    'id': 'c20959d2-daca-4cb2-a104-e1ab63479da3', 'title': 'Voices of a Distant Star', 'imdb_rating': 7.2,
    'description': 'The story of the high school students Mikako Nagamine and Noboru Terao. When the alien Tarsians '
                   'attack, Mikako volunteers to be a pilot in the space force that will protect mankind. '
                   'The lovers try to remain in contact using cellular telephone text messages, but as each '
                   'battle takes Mikako further from the Earth, each message takes longer to arrive. '
                   'Will their love stand the tests of time and distance?',
    'genre': [{'id': '1cacff68-643e-4ddd-8f57-84b62538081a', 'name': 'Drama'},
              {'id': '237fd1e4-c98e-454e-aa13-8a13fb7547b5', 'name': 'Romance'},
              {'id': '6a0a479b-cfec-41ac-b520-41b2b007b611', 'name': 'Animation'},
              {'id': '6c162475-c7ed-4461-9184-001ef3d9f26e', 'name': 'Sci-Fi'},
              {'id': 'a886d0ec-c3f3-4b16-b973-dedcf5bfa395', 'name': 'Short'}],
    'directors': [{'id': '74a4e5ce-5fa7-4f62-aad6-7b529e0cd96e', 'name': 'Makoto Shinkai'}],
    'actors': [{'id': '6c9ae6e9-43c3-4d5c-b857-49c26c61ffba', 'name': 'Chihiro Suzuki'},
               {'id': '74a4e5ce-5fa7-4f62-aad6-7b529e0cd96e', 'name': 'Makoto Shinkai'},
               {'id': 'ab890976-8022-464a-ad14-35de35bee244', 'name': 'Mika Shinohara'},
               {'id': 'f12a07e5-6da5-4c16-8b0f-c42f7b13c52f', 'name': 'Sumi Mutoh'}],
    'writers': [{'id': '74a4e5ce-5fa7-4f62-aad6-7b529e0cd96e', 'name': 'Makoto Shinkai'}]
}

DETAILED_FILM_DATA_PUT_TO_CACHE = {
    'id': '101424f3-e62b-4f6d-b797-7f3488a8f36c',
    'title': 'Python is the best of the best',
    'imdb_rating': 6.2,
    'description': 'Hello, world!',
    'genre': [{
        'id': '120a21cf-9097-479e-904a-13dd7198c1dd',
        'name': 'Adventure'
    }, {
        'id': '6c162475-c7ed-4461-9184-001ef3d9f26e',
        'name': 'Sci-Fi'
    }],
    'actors': [{
        'id': '26e83050-29ef-4163-a99d-b546cac208f8',
        'name': 'Mark Hamill'
    }, {
        'id': '2d6f6284-13ce-4d25-9453-c4335432c116',
        'name': 'Adam Driver'
    }, {
        'id': 'b5d2b63a-ed1f-4e46-8320-cf52a32be358',
        'name': 'Carrie Fisher'
    }],
    'writers': [{
        'id': '3217bc91-bcfc-44eb-a609-82d228115c50',
        'name': 'Lawrence Kasdan'
    }, {
        'id': 'a5a8f573-3cee-4ccc-8a2b-91cb9f55250a',
        'name': 'George Lucas'
    }, {
        'id': 'cec00f0e-200b-4b48-9ed1-2f8fc3c67427',
        'name': 'Michael Arndt'
    }],
    'directors': [{
        'id': 'a1758395-9578-41af-88b8-3f9456e6d938',
        'name': 'J.J. Abrams'
    }]
}

FILM_LIST_BY_GENRE_GOT_FROM_CACHE = [
    {'id': '134989c3-3b20-4ae7-8092-3e8ad2333d59', 'title': 'The Star Wars Holiday Special', 'imdb_rating': 2.1,
     'description': "It is Life Day, a holiday that is celebrated on Chewbacca's home planet Kashyyyk. "
                    "Chewie and Han Solo are trying to get to the planet where Chewie's family is waiting for him, "
                    'but the Empire is out searching for the Rebels, giving everyone a hard time. While we are '
                    'waiting we get a look at the everyday life of a Wookiee family. We meet all the familiar '
                    'characters from Star Wars: Episode IV - A New Hope (1977) and we are introduced to Boba '
                    'Fett during a short cartoon. We also pay a visit to the Cantina and meet all the monsters again.',
     'genre': [{'id': '120a21cf-9097-479e-904a-13dd7198c1dd', 'name': 'Adventure'},
               {'id': '55c723c1-6d90-4a04-a44b-e9792040251a', 'name': 'Family'},
               {'id': '6c162475-c7ed-4461-9184-001ef3d9f26e', 'name': 'Sci-Fi'},
               {'id': '9c91a5b2-eb70-4889-8581-ebe427370edd', 'name': 'Musical'}],
     'directors': [{'id': 'c97f0b2f-6cb0-459e-8547-46f8f4e7c1d7', 'name': 'David Acomba'},
                   {'id': 'dc448893-7942-4677-a557-d8b1aa728ccf', 'name': 'Steve Binder'}],
     'actors': [{'id': '26e83050-29ef-4163-a99d-b546cac208f8', 'name': 'Mark Hamill'},
                {'id': '5b4bf1bc-3397-4e83-9b17-8b10c6544ed1', 'name': 'Harrison Ford'},
                {'id': '86f1f44b-39f5-41a6-8b3b-af5a9ed09858', 'name': 'Anthony Daniels'},
                {'id': 'b5d2b63a-ed1f-4e46-8320-cf52a32be358', 'name': 'Carrie Fisher'}],
     'writers': [{'id': '3a79bcae-0fff-4aea-b841-d4ce74b8a2ff', 'name': 'Bruce Vilanch'},
                 {'id': '47c4a7d9-9fb6-444d-9852-56df2d3a98c1', 'name': 'Mitzie Welch'},
                 {'id': '70adcb88-ed89-4654-866e-9b124b95c82b', 'name': 'Rod Warren'},
                 {'id': '7f4382be-73f0-4741-bd46-6f895099b661', 'name': 'Pat Proft'},
                 {'id': 'a5a8f573-3cee-4ccc-8a2b-91cb9f55250a', 'name': 'George Lucas'},
                 {'id': 'f112018e-844a-49ba-a880-89ca8cc2389e', 'name': 'Leonard Ripps'}]}
]

FILM_LIST_BY_GENRE_PUT_TO_CACHE = [
    {'id': '13afddf3-7bd5-4b55-9cdd-6e90c623d288',
     'title': 'Java is not Python!',
     'imdb_rating': 1.5,
     'description': 'It is very sad to realize it',
     'genre': [{'id': '120a21cf-9097-479e-904a-13dd7198c1dd', 'name': 'Adventure'}],
     'directors': [{'id': 'c97f0b2f-6cb0-459e-8547-46f8f4e7c1d7', 'name': 'David Acomba'}],
     'actors': [{'id': '26e83050-29ef-4163-a99d-b546cac208f8', 'name': 'Mark Hamill'}],
     'writers': [{'id': 'f112018e-844a-49ba-a880-89ca8cc2389e', 'name': 'Leonard Ripps'}]
     }
]

FILM_LIST_BY_TITLE_GOT_FROM_CACHE = [
    {'id': '1eb9cc6b-879f-4160-8971-918ecbe47a87',
     'title': 'Star',
     'imdb_rating': 7.7,
     'description': 'The Driver now carries an arrogant rock star who is visiting a major city (not Pittsburgh as '
                    'earlier believed). Played by Madonna, this title character wants to get away from her '
                    "bodyguards in the Driver's BMW. He soon gets tired of her and decides to have a bit of fun.",
     'genre': [{'id': '3d8d9bf5-0d90-4353-88ba-4ccc5d2c07ff', 'name': 'Action'},
               {'id': '5373d043-3f41-4ea8-9947-4b746c601bbd', 'name': 'Comedy'},
               {'id': 'a886d0ec-c3f3-4b16-b973-dedcf5bfa395', 'name': 'Short'}],
     'directors': [{'id': '0ec9b6a8-27f9-47b2-8a51-15ae7a1a136d', 'name': 'Guy Ritchie'}],
     'actors': [{'id': '077ade46-00a6-4199-8c7a-43a84733b8ef', 'name': 'DTeflon'},
                {'id': '7a22eed2-50af-4e4d-aae8-6eee6cd27e07', 'name': 'Clive Owen'},
                {'id': '8ab1b385-c186-463b-8d97-30e67492aa60', 'name': 'Michael Beattie'},
                {'id': 'a35bd08b-acf0-4701-a967-92a0ca61ce62', 'name': 'Toru Tanaka Jr.'}],
     'writers': [{'id': '0ec9b6a8-27f9-47b2-8a51-15ae7a1a136d', 'name': 'Guy Ritchie'},
                 {'id': '261894a1-bbf6-4779-bd88-c121fdab661c', 'name': 'Joe Sweet'}]
     }
]

FILM_LIST_BY_TITLE_PUT_TO_CACHE = [
    {'id': 'f89b06b2-399f-4020-9a96-ee8e95d13d88',
     'title': 'Make php great again',
     'imdb_rating': 2.9,
     'description': "It's very hard by you should try",
     'genre': [{'id': '3d8d9bf5-0d90-4353-88ba-4ccc5d2c07ff', 'name': 'Action'}],
     'directors': [{'id': '0ec9b6a8-27f9-47b2-8a51-15ae7a1a136d', 'name': 'Guy Ritchie'}],
     'actors': [{'id': '077ade46-00a6-4199-8c7a-43a84733b8ef', 'name': 'DTeflon'}],
     'writers': [{'id': '0ec9b6a8-27f9-47b2-8a51-15ae7a1a136d', 'name': 'Guy Ritchie'}]
     }
]
